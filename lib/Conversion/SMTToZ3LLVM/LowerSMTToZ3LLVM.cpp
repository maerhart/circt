...
/// Some the Z3 API supports a variadic number of operands for some operations
/// (in particular if the expansion would lead to a super-linear increase in
/// operations such as with the ':pairwise' attribute). Those API calls take
/// an 'unsigned' argument indicating the size of an array of pointers to the
/// operands.
...
/// Lower `smt.assert` operations Z3 API calls of the form:
/// ```
