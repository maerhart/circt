  ...
  WalkResult result =
      module->walk([](ProceduralInstanceInteropOpInterface op) -> WalkResult {
        ...
        InteropAllocFunc allocator =
            [&](OpBuilder &builder) -> Optional<SmallVector<Value>> {
          return op.allocState(builder, interopType);
        };
        InteropUpdateFunc updater =
            [&](ArrayRef<Value> state,
                ArrayRef<Value> input) -> SmallVector<Value> {
          OpBuilder builder(op);
          return op.updateState(builder, state, input, interopType);
        };
        SmallVector<Value> inputs(op->getOperands());
        auto newValues =
            parent.wrapInterop(op.getRequiredState(interopType), inputs,
