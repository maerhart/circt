...
struct WaitEventOpConversion : public OpConversionPattern<WaitEventOp> {
  ...
  matchAndRewrite(WaitEventOp op, OpAdaptor adaptor,
    ...
    auto computeTrigger = [&](Value before, Value after, Edge edge) -> Value {
      ...
      auto targetType = typeConverter->convertType(before.getType());
      before = typeConverter->materializeTargetConversion(rewriter, loc,
                                                          targetType, before);
      after = typeConverter->materializeTargetConversion(rewriter, loc, targetType, after);

      if (!isa<IntegerType>(targetType)) {
        auto bw = hw::getBitWidth(targetType);
        assert(bw >= 0 && "must be valid type");
        before = rewriter.create<hw::BitcastOp>(op.getLoc(), rewriter.getIntegerType(bw), before);
        after = rewriter.create<hw::BitcastOp>(op.getLoc(), rewriter.getIntegerType(bw), after);
      }
...
struct ExtractOpConversion : public OpConversionPattern<ExtractOp> {
  ...
  matchAndRewrite(ExtractOp op, OpAdaptor adaptor,
    ...
    if (isa<IntegerType>(inputType)) {
      if (adaptor.getLowBit() < 0 || adaptor.getLowBit() >= inputType.getIntOrFloatBitWidth()) {
        // TODO: return X when the domain is four-valued
        rewriter.replaceOpWithNewOp<hw::ConstantOp>(op, resultType, 0);
        return success();
      }
