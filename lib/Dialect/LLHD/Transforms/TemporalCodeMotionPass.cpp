...
static Value getSourceToTargetPathsCondition(OpBuilder &builder, Block *source,
  ...
  return pathCollectorDFS(builder, target, source, mem, visited);
}
...
static void moveDrvInBlock(DrvOp drvOp, Block *dominator,
  ...
  Value finalValue = getSourceToTargetPathsCondition(builder, dominator, drvParentBlock, mem);

  if (drvOp.getOperation()->getNumOperands() == 4) {
    finalValue =
        builder.create<comb::AndOp>(drvOp.getLoc(), drvOp.enable(), finalValue);
...
struct TemporalCodeMotionPass
    : public llhd::TemporalCodeMotionPassBase<TemporalCodeMotionPass> {
...
void TemporalCodeMotionPass::runOnOperation() {
  ...
  //===--------------------------------------------------------------------===//
  // Create unique exiting block per Temporal Region
  //===--------------------------------------------------------------------===//
  ...
  for (int currTR = 0; currTR < (int)numTRs; ++currTR) {
    ...
    for (Type ty : succTREntry->getArgumentTypes()) {
      auxBlock->addArgument(ty, proc->getLoc());
    }
    ...
    b.create<cf::BranchOp>(proc.getLoc(), succTREntry, auxBlock->getArguments());
  }
      ...
      if (!sigToDrv.count(sigTimePair)) {
        ...
        if (op.enable()) {
          ...
          if (op.value() != sigToDrv[sigTimePair].value()) {
            ...
            Value condition = getSourceToTargetPathsCondition(
                builder, dominator, drvPos[op]);
            drvPos[op] = dominator;
            Value muxValue =
                builder.create<comb::MuxOp>(op.getLoc(), condition, op.value(),
          ...
          if (sigToDrv[sigTimePair].enable()) {
            Value orVal = builder.create<comb::OrOp>(
                op.getLoc(), op.enable(),
