...
LogicalResult UnrollPass::runOnProcess(llhd::ProcessOp procOp) {
  ...
  //   // NOTE: Above error checks make this impossible to trigger, but the
  //   above
  //   // are changed this one might have to be promoted to a proper error
  //   message.
  ...
  //       SmallVector<Location>(succTREntry->getNumArguments(),
  //       procOp.getLoc()));
  ...
  // for (auto *block : trAnalysis.getBlocksInTR(i)) {
  for (auto &block : procOp.getBody().getBlocks()) {
    ...
    if (!condBr) {
      // block.getTerminator()->emitWarning("expected cond_br");
      continue;
    }

    int tripCount = getTripCount(condBr.getCondition());
    if (tripCount == -1) {
      // condBr->emitWarning("could not determine trip count");
      continue;
    }

    auto loopPathGoesThroughWait = [](Block *b, Block *stopAt) {
      DenseSet<Block *> seen;
      SmallVector<Block *> worklist;
    ...
    while (!worklist.empty()) {
      ...
      if (!seen.contains(curr)) {
        seen.insert(curr);
        if (!loopPathGoesThroughWait(curr, &block)) {
    ...
    // block.getTerminator()->emitRemark("performing unrolling");
