...
bool circt::llhd::Dnf::isProbedSignal() {
  return isVal() && isa<llhd::PrbOp>(value.getDefiningOp());
}

Value circt::llhd::Dnf::getProbedSignal() {
...
void circt::llhd::Dnf::simplifyExpression() {
...
static mlir::Value createConstOrValueDNF(mlir::OpBuilder &builder,
  ...
  if (dnf.isConst()) {
    return builder.create<circt::hw::ConstantOp>(
        loc, builder.getI1Type(), builder.getBoolAttr(dnf.getConst()));
  }
  if (dnf.isNegatedVal()) {
    Value allset = builder.create<hw::ConstantOp>(loc, dnf.value.getType(), -1);
    return builder.create<comb::XorOp>(loc, dnf.value, allset);
  }
  if (dnf.isVal()) {
    return dnf.value;
  }
...
static mlir::Value createLeafOrAndDNF(mlir::OpBuilder &builder,
  ...
  if (dnf.isAnd()) {
    ...
    for (std::unique_ptr<circt::llhd::Dnf> &term : dnf.children) {
      if (!runner) {
        runner = createConstOrValueDNF(builder, *term);
        continue;
      }
      runner = builder.createOrFold<circt::comb::AndOp>(
          loc, runner, createConstOrValueDNF(builder, *term));
    }
    return runner;
  }
  return mlir::Value();
...
static std::unique_ptr<circt::llhd::Dnf> &
  ...
  for (auto iter = curr->pred_begin(); iter != curr->pred_end(); ++iter) {
    if ((*iter)->getTerminator()->getNumSuccessors() == 1) {
      ...
      llhd::Dnf tmp(llhd::DnfNodeType::And,
                    std::make_unique<llhd::Dnf>(llhd::Dnf(
                        *recursiveHelper(*iter, source, mem, visited))),
                    std::make_unique<llhd::Dnf>(condBr.getCondition(),
                                                condBr.getFalseDest() == curr));
      if (!dnf)
...
Value circt::llhd::Dnf::buildOperations(OpBuilder &builder) {
  ...
  if (isOr()) {
    mlir::Value runner = mlir::Value();
    for (std::unique_ptr<circt::llhd::Dnf> &term : children) {
      if (!runner) {
        runner = createLeafOrAndDNF(builder, *term);
        continue;
      }
      runner = builder.create<circt::comb::OrOp>(
...
circt::llhd::getBooleanExprFromSourceToTarget(Block *source, Block *target) {
  ...
  for (auto iter = curr->pred_begin(); iter != curr->pred_end(); ++iter) {
    if ((*iter)->getTerminator()->getNumSuccessors() == 1) {
      ...
      if (!runner)
        ...
        runner = builder.create<comb::OrOp>(loc, runner, tmp);
    }
  }
