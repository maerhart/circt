...
unsigned circt::llhd::getLLHDTypeWidth(Type type) {
  if (auto sig = type.dyn_cast<hw::InOutType>())
...
static LogicalResult inferReturnTypesOfStructExtractOp(
    MLIRContext *context, std::optional<Location> loc, Type structTy,
    DictionaryAttr attrs, mlir::OpaqueProperties properties,
    mlir::RegionRange regions, SmallVectorImpl<Type> &results) {
  Type type = structTy.cast<hw::StructType>().getFieldType(
...
LogicalResult llhd::SigStructExtractOp::inferReturnTypes(
  ...
  return inferReturnTypesOfStructExtractOp<hw::InOutType>(
      context, loc,
      operands[0].getType().cast<hw::InOutType>().getElementType(), attrs,
...
LogicalResult llhd::PtrStructExtractOp::inferReturnTypes(
  ...
  return inferReturnTypesOfStructExtractOp<llhd::PtrType>(
      context, loc, operands[0].getType().cast<PtrType>().getUnderlyingType(),
      attrs, properties, regions, results);
}
...
LogicalResult circt::llhd::ProcOp::verifyType() {
  ...
  for (int i = 0, e = getNumArguments(); i < e; ++i) {
    if (!getArgument(i).getType().isa<hw::InOutType>()) {
      return emitOpError("usage of invalid argument type, was ")
             << getArgument(i).getType() << ", expected HW inout type";
    }
  }
  return success();
}
...
LogicalResult llhd::InstOp::verify() {
  ...
  return emitOpError() << "'" << calleeAttr.getValue()
                       << "' does not reference a valid proc";
}
...
LogicalResult llhd::RegOp::verify() {
  ...
  if (getTriggers().size() < 1)
    return emitOpError("At least one trigger quadruple has to be present.");
  ...
  for (auto val : getValues()) {
    if (val.getType() != getSignal().getType() &&
        val.getType() !=
            getSignal().getType().cast<hw::InOutType>().getElementType()) {
      return emitOpError(
