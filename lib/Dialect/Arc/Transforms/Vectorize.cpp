...
void VectorizePass::runOnOperation() {
  getOperation()->walk([&](ClockDomainOp clockDomain) {
    Block *block = &clockDomain.getBodyBlock();
    llvm::MapVector<StringAttr, SmallVector<StateOp>> grouping;
    auto stateops = block->getOps<StateOp>();
    llvm::outs() << "starting with "
                 << std::distance(stateops.begin(), stateops.end()) << "\n";
    ...
    for (auto [_, states] : grouping) {
      llvm::outs() << "have" << states.size() << "elements in group\n";
      if (states.size() <= 1)
        continue;
      ...
      auto parallelOp = builder.create<VectorizeOp>(
          fusedLoc,
      ...
      builder.create<VectorizeReturnOp>(fusedLoc, states[0]->getResult(0));
...
std::unique_ptr<Pass> circt::arc::createVectorizePass() {
