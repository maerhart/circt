...
// CHECK-LABEL: llhd.process @check_dont_move_sideeffect
// CHECK-SAME:  (inout %[[VAL_0:[[:alnum:]]+]]{{.*}}: i32)
...
// CHECK:           llhd.halt
// CHECK:         }
llhd.process @check_dont_move_sideeffect(inout %sig : i32) {
  // TR: -1
  %c = hw.constant 4 : i32
...
// are moved freely
// CHECK-LABEL: llhd.process @check_move_prb1
// CHECK-SAME:  (inout %[[VAL_0:[[:alnum:]]+]]{{.*}} : i32)
// CHECK:           %[[VAL_1:.*]] = hw.constant 4 : i32
...
// CHECK:           llhd.wait ^bb1
// CHECK:         }
llhd.process @check_move_prb1(inout %sig : i32) {
  // TR: -1
  cf.br ^bb1
^bb1:
  // TR: 0
...
// CHECK-LABEL: llhd.process @check_move_prb2
// CHECK-SAME:  (inout %[[VAL_0:[[:alnum:]]+]]{{.*}} : i32)
// CHECK:           %[[VAL_1:.*]] = hw.constant 4 : i32
// CHECK:           %[[VAL_2:.*]] = comb.add %[[VAL_1]], %[[VAL_1]] : i32
// CHECK:           cf.br ^bb1
...
// CHECK:           cf.br ^bb1
// CHECK:         }
llhd.process @check_move_prb2(inout %sig : i32) {
  // TR: -1
  cf.br ^bb1
^bb1:
  // TR: 1
...
// CHECK-LABEL: llhd.process @check_blockarg
// CHECK-SAME:  (inout %[[VAL_0:[[:alnum:]]+]]{{.*}} : i32)
// CHECK:           %[[VAL_1:.*]] = hw.constant 4 : i32
...
// CHECK:           llhd.halt
// CHECK:         }
llhd.process @check_blockarg(inout %sig : i32) {
  // TR: -1
  %c = hw.constant 4 : i32
  cf.br ^bb1(%c : i32)
^bb1(%a : i32):
...
// CHECK-LABEL: llhd.process @loop
// CHECK-SAME:  (inout %[[VAL_0:[[:alnum:]]+]]{{.*}} : i2)
...
// CHECK:           cf.br ^bb2
// CHECK:         }
llhd.process @loop(inout %in_i : i2) {
  // TR: -1
  cf.br ^body
^body:
  // TR: 0
...
// CHECK-LABEL: llhd.process @complicated
// CHECK-SAME:  (inout %[[VAL_0:[[:alnum:]]+]]{{.*}} : i1, inout %[[VAL_1:[[:alnum:]]+]]{{.*}} : i1, inout %[[VAL_2:[[:alnum:]]+]]{{.*}} : i1, inout %[[VAL_3:[[:alnum:]]+]]{{.*}} : i1, inout %[[VAL_4:[[:alnum:]]+]]{{.*}} : i1)
...
// CHECK:           cf.br ^bb1
// CHECK:         }
llhd.process @complicated(inout %rst_ni: i1, inout %clk_i: i1, inout %async_ack_i: i1, inout %ack_src_q: i1, inout %ack_q: i1) {
